...
const keywordDeepDives = {
    ...
    spanish: {
        ...
        sum_t: {
            ...
            content: `La suma resuelta \\(\\sum_{j=1}^{i-1} a_{ij} x_j^{(k+1)}\\) representa las fuerzas o impulsos acumulados de todas las restricciones conectadas que ya han sido actualizadas en el barrido actual del resolvedor. Esta es la retroalimentación de las personas por encima de B1 que tiran de él hacia arriba. En una CPU, el código se ejecuta en un bucle <code>for</code> estricto de arriba a abajo. Los escaladores por encima de B1 (T1, T2, T3) están antes en el arreglo de memoria. La CPU ya resolvió su matemática para este fotograma. Por lo tanto, al calcular B1, el motor lee su <code>linearVelocity</code> actualizada directamente de la caché L1.<br><br>Los motores de juego dependen de un truco llamado <code>warm starting</code> (inicio en caliente). Alimentan las fuerzas de tensión exactas del fotograma anterior directamente en el actual, dándole a la CPU una ventaja masiva para resolver la cadena de articulaciones.`
        },
        sum_b: {
            title: "Suma No Resuelta \\(\\sum_{j=i+1}^{n} a_{ij} x_j^{(k)}\\)",
            content: `La suma no resuelta \\(\\sum_{j=i+1}^{n} a_{ij} x_j^{(k)}\\) representa las fuerzas de las restricciones conectadas que aún no se han actualizado en la iteración del resolvedor actual, lo que obliga al motor a utilizar los valores del paso anterior (\\(k\\)). Esto coincide con el arrastre de los escaladores que cuelgan debajo de B1. La CPU aún no ha llegado a ellos en el arreglo. Su matemática no se ha actualizado. Entonces, ¿qué hace el motor? Simplemente lee la <code>linearVelocity</code> de esos escaladores inferiores del subpaso de física anterior (el estado \\(k\\)).<br><br>Utiliza datos obsoletos. Es una suposición informada. Porque si el motor se detuviera para calcularlos perfectamente en el mismo nanosegundo, el hilo de hardware se bloquearía y el juego se colgaría.`
        },
        a_ij: {
            title: "Coeficiente de Acoplamiento \\(a_{ij}\\)",
            content: `El coeficiente de acoplamiento \\(a_{ij}\\) representa la fuerza de interacción física o la conexión de restricción entre dos cuerpos, como la rigidez de una articulación de conexión. En el desarrollo de juegos real, no tienes manos humanas. Tienes componentes. Cuando construyes un ragdoll en Unity o Unreal Engine, conectas los brazos y las piernas usando un <code>ArticulationBody</code> o un <code>HingeJoint</code>. Este coeficiente \\(a_{ij}\\) se controla directamente mediante los números de rigidez (<code>Stiffness</code>) y amortiguación (<code>Damping</code>) que escribes manualmente en el panel del inspector. Alta rigidez significa que el motor obliga a la matemática a transferir el 100% de la fuerza de tracción instantáneamente.`
        },
        temporal: {
            title: "Coherencia Temporal (Historial de Tiempo)",
            content: `La coherencia temporal es el principio de reciclar los estados de velocidad y posición del fotograma anterior para inicializar los cálculos del fotograma actual, ahorrando valiosos ciclos de CPU. La computadora no empieza de cero. Eso desperdiciaría demasiada potencia de procesamiento. Piénsalo: 1/60 de segundo es prácticamente nada. Los objetos apenas se mueven en ese tiempo. Por lo tanto, el motor utiliza la coherencia temporal. Analiza el estado exacto de la memoria del último fotograma y lo recicla.<br><br>Toma los impulsos matemáticos finales de hace 16 milisegundos y literalmente los copia y pega como punto de partida para el nuevo fotograma. Para nuestros escaladores, esto significa que en lugar de volver a calcular todo su peso corporal y posición desde cero, el motor los inicia exactamente donde terminaron hace un milisegundo. Y luego solo hace pequeños ajustes.`
        },
        mobility: {
            title: "Movilidad (Inercia Inversa)",
            content: `La movilidad es la inversa matemática de la masa (\\(1/\\text{masa}\\)). Precalcular la movilidad permite a la CPU reemplazar las operaciones de división lentas con una multiplicación rápida para calcular ajustes rápidamente. Las cosas pesadas son difíciles de mover. Las cosas ligeras son fáciles de mover. En una ecuación de física normal, se divide por la masa para calcular el movimiento. Pero aquí hay un problema con las computadoras: las CPU odian la división. La división consume demasiados ciclos de reloj eléctrico. Por lo tanto, el motor utiliza la movilidad en su lugar.<br><br>La movilidad es simplemente el opuesto matemático de la masa. Es uno dividido por la masa. El motor calcula esto una vez y lo guarda. Ahora, en lugar de dividir, la CPU simplemente usa la multiplicación. Alta movilidad significa que un objeto es increíblemente ligero. Baja movilidad significa que es pesado. Si B1 tiene alta movilidad, sus coordenadas matemáticas son físicamente más fáciles de mover para el procesador.`
        },
        unbalanced: {
            title: "Fuerza Desequilibrada (Residual)",
            content: `Una fuerza desequilibrada (error residual) ocurre cuando la suma de las fuerzas opuestas que actúan sobre un cuerpo no es igual a cero. Por ejemplo, si la gravedad tira de B1 hacia abajo con 100 unidades de fuerza, pero los escaladores de arriba tiran hacia arriba con solo 90, te quedan 10 unidades de fuerza tirando hacia abajo. Ese sobrante de 10 es la fuerza desequilibrada. Es literalmente un error matemático en la ecuación. Y el motor no puede dejar un error simplemente ahí.`
        },
        slip: {
            title: "Deslizamiento (Desplazamiento de Corrección)",
            content: `El deslizamiento representa el desplazamiento de corrección física o el ajuste de velocidad aplicado a un cuerpo para resolver una fuerza desequilibrada. Se calcula multiplicando la fuerza desequilibrada por la movilidad del cuerpo. Toma esa fuerza desequilibrada de 10 de la que acabamos de hablar y la multiplica por la movilidad de B1. El resultado es un desplazamiento de velocidad muy específico. Es un número exacto de píxeles o unidades de coordenadas que el objeto necesita moverse para cancelar el error. En la simulación, esto parece como si B1 deslizara sus manos por el borde del acantilado. Sus coordenadas matemáticas cambian. Se mueve a una nueva posición estable donde las fuerzas finalmente se equilibran a cero.`
        },
        sub_steps: {
            title: "Subpasos (División de Tiempo)",
            content: `Un bucle de juego suele ejecutarse a 60 fotogramas por segundo. Eso significa que el motor tiene un delta de tiempo (\\(\\Delta t\\)) de 0.016 segundos para calcular todo. La ecuación física estándar es: <code>Nueva Posición = Antigua Posición + (Velocidad * \\Delta t)</code>.<br><br>Pero si la velocidad de un objeto es increíblemente alta, multiplicarla por 0.016 da como resultado un salto masivo en la posición. El objeto literalmente se teletransporta del punto A al punto B en la memoria. Si existe una pared de acantilado entre el punto A y el punto B, el motor nunca registrará una colisión.<br><br>Cuando aumentas los subpasos (<code>sub_steps</code>) a 4, estás obligando a la CPU a dividir ese delta de 0.016s por 4. La CPU ahora ejecuta físicamente todo el proceso de física cuatro veces distintas por fotograma, utilizando un \\(\\Delta t\\) de 0.004s. El objeto se mueve en pequeños incrementos seguros. ¿La desventaja? Estás exigiendo exactamente 4 veces más ciclos de reloj de cálculo al procesador.`
        },
        iterations: {
            title: "Iteraciones (Pasadas de Relajación)",
            content: `Las iteraciones en un resolvedor de física son pasadas secuenciales utilizadas para resolver restricciones interconectadas, donde el ajuste de una restricción puede desequilibrar temporalmente otra. ¿Por qué necesitamos repetir la matemática? Porque las restricciones de física están conectadas. Si empujas a B1 hacia arriba para cumplir con una articulación, estiras accidentalmente la articulación que conecta a B1 con B2. Corregir la restricción A rompe la restricción B.<br><br>Cuando configuras las iteraciones (<code>iterations</code>) en 10, la CPU busca el arreglo de restricciones de la RAM en su caché L1. Resuelve A, luego B, luego C. Para cuando termina C, A vuelve a estar ligeramente mal. Entonces, la CPU vuelve a ejecutar la matemática por segunda vez. Cada vez que la CPU realiza un barrido de Gauss-Seidel sobre el arreglo de memoria, el error residual se vuelve más y más pequeño. Las variables convergen físicamente hacia los números correctos. En la iteración 10, el error es tan pequeño que el ojo humano percibe la articulación como acero completamente rígido.`
        },
        static: {
            title: "BodyType::Static",
            content: `A veces necesitas un suelo o una pared de acantilado que nunca se mueva. Jamás. En el código, lo etiquetas como un cuerpo estático. Esto le dice al motor de física que ignore por completo este objeto al calcular cosas como la gravedad o el momento. Si un objeto dinámico masivo choca contra él a un millón de kilómetros por hora, el cuerpo estático no se mueve. Es un ancla inamovible que representa una masa infinita.`
        },
        inv_mass: {
            title: "Masa Inversa (1/M)",
            content: `La masa inversa (\\(1/\\text{mass}\\)) se establece exactamente en 0.0 para los cuerpos estáticos para evitar que se muevan. Este truco matemático evita los retrasos de predicción de ramas de la CPU causados por las comprobaciones condicionales 'if'. ¿Por qué no escribir simplemente una instrucción 'if' en el código para evitar que esa pared estática se mueva? Porque los procesadores de computadora odian las instrucciones 'if'. Hacer una pregunta obliga al hardware físico a adivinar la respuesta, y si adivina mal, tiene que descartar todos sus cálculos y empezar de nuevo. Eso arruina el rendimiento. Entonces, en lugar de usar 'if', el motor usa <code>inv_mass</code>. Para una pared estática, el motor codifica deliberadamente ese número de <code>inv_mass</code> exactamente en 0.0. Entonces, cuando otro objeto golpea la pared, el motor simplemente multiplica la fuerza de choque entrante por cero. El movimiento resultante es cero. La CPU procesa la matemática directamente sin tener que detenerse, adivinar o preguntar.<br><br>A nivel de silicio, multiplicar el voltaje de la fuerza por cero produce un voltaje de salida de cero en el registro de velocidad. La velocidad de la pared sigue siendo exactamente cero. La CPU ejecuta la matemática sin tener que pausar y preguntar "¿es esto una pared?", eliminando por completo los retrasos de predicción de ramas.`
        },
        dynamic: {
            title: "BodyType::Dynamic",
            content: `Un cuerpo dinámico es un objeto físico completamente simulado con masa real, distinta de cero, que responde a la gravedad, las colisiones y las restricciones de articulación. La gravedad lo agarra y tira de él hacia abajo. Cuando las cosas lo golpean, rebota. Cuando una articulación tira de él, se mueve. El motor tiene que calcular cada fuerza física y regla de fricción para estos cuerpos dinámicos cada vez que la pantalla dibuja un fotograma.`
        },
        update: {
            title: "Ciclo de Actualización del Mundo",
            content: `Cuando el bucle principal del juego llama a la actualización, activa una secuencia muy estricta de instrucciones de hardware.<br><br><b>Broadphase</b>: La CPU comprueba cajas de límite simples e invisibles alrededor de cada objeto. Si las cajas no se superponen, la CPU las descarta de inmediato para ahorrar potencia de procesamiento.<br><br><b>Narrowphase</b>: Para las cajas que sí se superponen, la CPU ejecuta algoritmos geométricos intensos (como GJK o EPA) para encontrar la coordenada exacta del punto de contacto con precisión de píxel.<br><br><b>Solver</b>: Toma esos puntos de contacto, crea las ecuaciones matemáticas y ejecuta el bucle de iteraciones que analizamos anteriormente.<br><br><b>Integration</b>: Finalmente, la CPU toma las velocidades resueltas y sobrescribe las coordenadas de posición X, Y y Z en la RAM de la computadora.`
        },
        constraints: {
            title: "Recuento de Restricciones",
            content: `Una restricción es simplemente una regla matemática estricta que el motor tiene que resolver. Si dos cajas se tocan entre sí, ese punto de contacto es una restricción. Es una regla que dice 'estas dos cosas no pueden ocupar el mismo espacio'. Si vinculas dos cajas con una cuerda, esa es una restricción de articulación. Dice 'estas dos cosas no pueden estar a más de metro y medio de distancia'. El motor lleva un recuento de cada regla. Funciona como un número de telemetría para mostrarte exactamente cuánta presión está ejerciendo tu sistema de física sobre el procesador en este momento.`
        },
        batches: {
            title: "Lotes de Ejecución",
            content: `Los lotes de ejecución dividen grupos independientes de objetos en colisión (islas) para que se ejecuten simultáneamente. Las CPU modernas tienen múltiples núcleos (Núcleo 1, Núcleo 2, Núcleo 3). Quieres que todos funcionen a la vez. Pero existe un riesgo llamado carrera de datos. Si el Núcleo 1 está calculando el brazo izquierdo del escalador B1 y el Núcleo 2 está calculando el brazo derecho de B1, ambos podrían intentar escribir una nueva posición en la misma dirección de memoria de B1 en la RAM en el mismo nanosegundo. El hardware entra en pánico y los datos se corrompen.<br><br>Para evitar esto, el motor analiza el mapa antes de resolverlo. Busca islas físicas: grupos de objetos que se tocan entre sí, pero que están completamente separados de otros grupos. Esto son lotes. El motor envía la Isla de Escaladores al Núcleo 1 y una Isla de Choque de Autos completamente separada al Núcleo 2. Debido a que las direcciones de memoria de los escaladores y los autos están físicamente separadas en la RAM, los núcleos pueden funcionar al 100% de velocidad sin sobrescribirse accidentalmente los datos entre sí.`
        },
        simd: {
            title: "Vectorización SIMD",
            content: `Normalmente, una CPU procesa una instrucción matemática a la vez: carga dos números, los suma y genera el resultado. Pero el silicio moderno tiene registros de hardware anchos llamados registros SIMD (Instrucción Única, Datos Múltiples). En lugar de procesar una ecuación a la vez, SIMD permite al motor empaquetar cuatro u ocho ecuaciones de restricciones una al lado de la otra.<br><br>La CPU ejecuta una sola instrucción eléctrica y resuelve las ocho ecuaciones en el mismo ciclo de reloj. Hace ocho veces más trabajo por el mismo costo eléctrico. Esta telemetría rastrea con qué éxito se procesaron las restricciones en estas canalizaciones de vectores de hardware.`
        }
    }
};
